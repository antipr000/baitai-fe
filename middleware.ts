...
const MAIN_PUBLIC_PATHS = ['/', '/about', '/pricing', '/founders', '/privacy', '/support', ...CANDIDATE_AUTH_PAGES];
...
export async function middleware(request: NextRequest) {
  ...
  return authMiddleware(request, {
    ...
    handleValidToken: async ({ token, decodedToken }, headers) => {
      ...
      if (isTeam && !pathname.startsWith('/company') && !pathname.startsWith('/api') && !pathname.startsWith('/results/') && !pathname.startsWith('/billing/')) {
        return NextResponse.redirect(new URL('/company/dashboard', request.url));
      }
