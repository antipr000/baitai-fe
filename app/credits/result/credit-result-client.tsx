...
function CreditResultContent({ authToken }: { authToken?: string }) {
  ...
  const verify = useCallback(async (sid: string, attempt = 1) => {
    try {
      const res = await verifyUserSession(sid, authToken);
...
export function CreditResultClient({ authToken }: { authToken?: string }) {
  return (
    <Suspense fallback={
      ...
      <CreditResultContent authToken={authToken} />
    </Suspense>
  );
}
