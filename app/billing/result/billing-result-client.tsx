...
function BillingResultContent({ authToken }: { authToken?: string }) {
  ...
  const verify = useCallback(async (sid: string, attempt = 1) => {
    try {
      const res = await verifyCompanySession(sid, authToken);
      ...
      <div className="bg-white rounded-2xl border border-[rgba(58,63,187,0.15)] p-8 md:p-12 max-w-md w-full text-center flex flex-col items-center gap-4">
        ...
        {state === "cancelled" && (
          <>
            ...
            <Link href="/company/pricing" className="mt-4">
              <Button className="bg-[rgba(58,63,187,1)] hover:bg-[rgba(58,63,187,0.9)] text-white font-semibold px-6">
                Back to Pricing
              </Button>
            </Link>
...
export function BillingResultClient({ authToken }: { authToken?: string }) {
  return (
    <Suspense fallback={
      ...
      <BillingResultContent authToken={authToken} />
    </Suspense>
  );
}
